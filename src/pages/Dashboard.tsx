...
export default function LiveCommittee() {
  const { activeCommittee, addNote, addTimelineEvent, addAlert, addSnapshot, updateContextPack, addBlocEntry } = useApp();
  ...
  const advanceSpeaker = async () => {
    if (queue.length === 0 && !currentSpeaker) {
      await addAlert({ severity: "info", title: "Speaker queue is empty", description: "Add delegates to the queue before advancing." });
      return;
    }
    if (!currentSpeaker) {
  ...
  const quickRel = async (stance: "ally" | "opponent" | "swing") => {
    ...
    if (existing) {
      ...
      await addBlocEntry({ ...existing, stance, supportLevel: support });
  ...
  const setPhase = (p: CommitteePhase) => {
    ...
    updateContextPack({ ...activeCommittee.contextPack, current_phase: p });
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="bg-gradient-to-r from-red-950/40 via-gray-900 to-gray-900 border border-red-800/40 rounded-lg p-3 flex items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          <span className="text-2xl">{ctx.country ? "🏳️" : "🌍"}</span>
          <div>
      ...
      <div className="col-span-12 md:col-span-4 bg-gray-900 border border-gray-800/60 rounded-lg p-3">
        ...
        <div className="space-y-3">
          <TimerBlock label="Motion" seconds={motionSeconds} running={motionRunning}
            onStart={() => setMotionRunning(true)} onStop={() => setMotionRunning(false)} onReset={() => { setMotionRunning(false); setMotionSeconds(0); }} />
          <TimerBlock label="Caucus" seconds={caucusSeconds} running={caucusRunning}
            onStart={() => setCaucusRunning(true)} onStop={() => setCaucusRunning(false)} onReset={() => { setCaucusRunning(false); setCaucusSeconds(0); }} />
        </div>
      </div>
      ...
      {/* ANALYSIS TOOLS ROW */}
      <div className="col-span-12 bg-gray-900 border border-gray-800/60 rounded-lg p-3 space-y-3">
        <div className="flex items-center justify-between gap-2">
          <div className="text-[10px] font-semibold text-gray-300 uppercase tracking-wider flex items-center gap-1.5">
            <div className="w-1 h-1 rounded-full bg-fuchsia-400 animate-pulse" /> Tactical Analysis Suite
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <div className="bg-gray-800/40 border border-white/6 rounded-lg p-3 space-y-2">
            <div className="text-[10px] font-semibold text-gray-300 uppercase tracking-wider flex items-center gap-1.5">
              <div className="w-1 h-1 rounded-full bg-amber-400" /> Agenda Ontology Active
            </div>
            <div className="space-y-1.5 text-[10px] text-gray-400">
              <div className="flex justify-between"><span>Sovereignty Weight</span><span className="text-amber-400 font-mono">0.90</span></div>
              <div className="flex justify-between"><span>Intervention Tolerance</span><span className="text-red-400 font-mono">0.30</span></div>
              <div className="flex justify-between"><span>Humanitarian Weight</span><span className="text-emerald-400 font-mono">0.70</span></div>
              <div className="flex justify-between"><span>Proxy Warfare Weight</span><span className="text-red-400 font-mono">0.95</span></div>
              <div className="flex justify-between"><span>Bloc Cohesion</span><span className="text-blue-400 font-mono">0.80</span></div>
              <div className="flex justify-between"><span>Swing Volatility</span><span className="text-amber-400 font-mono">0.60</span></div>
            </div>
            <div className="text-[9px] text-gray-500 bg-gray-900/40 rounded p-2 mt-1">
              💡 This agenda heavily weights sovereignty and proxy warfare. Use humanitarian framing to gain leverage. Avoid enforcement language — it will alienate G77/NAM.
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="bg-gray-800/40 rounded-md px-3 py-2">
      ...
      <div className="flex gap-1 mt-1.5">
        {!running ? (
          ...
          <button onClick={onStop} className="flex-1 py-1 bg-amber-600/20 hover:bg-amber-600/30 text-amber-300 border border-amber-500/30 rounded text-[10px] font-medium">#ause</button>
        )}
